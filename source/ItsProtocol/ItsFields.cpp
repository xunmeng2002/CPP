...
ItsReqQryPrimaryAccountInfo::ItsReqQryPrimaryAccountInfo(const vector<string>& items)
{
	...
	if(items.size() < 11)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsReqQryPrimaryAccountInfo's ColNum:[11]", items.size());
		return;
	}
...
ItsRspQryPrimaryAccountInfo::ItsRspQryPrimaryAccountInfo(const vector<string>& items)
{
	...
	if(items.size() < 10)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsRspQryPrimaryAccountInfo's ColNum:[10]", items.size());
		return;
	}
	ProtocolType = items[0];
	SequenceNo = items[1];
...
ItsInsertOrder::ItsInsertOrder(const vector<string>& items)
{
	...
	if(items.size() < 26)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsInsertOrder's ColNum:[26]", items.size());
		return;
	}
	ProtocolType = items[0];
	Command = items[1];
...
{
	return snprintf(buff, size, "ItsInsertOrder: ProtocolType[%s] Command[%s] SequenceNo[%s] Reserve1[%s] Reserve2[%s] Reserve3[%s] Reserve4[%s] Reserve5[%s] Reserve6[%s] ExchangeID[%s] ContractID[%s] Price[%s] Direction[%s] Reserve7[%s] CombHedgeFlag[%s] Volume[%s] OrderPriceType[%s] RequestID[%s] Reserve8[%s] TimeCondition[%s] GTDDate[%s] VolumeCondition[%s] MinVolume[%s] IsSwapOrder[%s] ForceCloseReason[%s] AccountID[%s] TradingDay[%s] ", ProtocolType.c_str(), Command.c_str(), SequenceNo.c_str(), Reserve1.c_str(), Reserve2.c_str(), Reserve3.c_str(), Reserve4.c_str(), Reserve5.c_str(), Reserve6.c_str(), ExchangeID.c_str(), ContractID.c_str(), Price.c_str(), Direction.c_str(), Reserve7.c_str(), CombHedgeFlag.c_str(), Volume.c_str(), OrderPriceType.c_str(), RequestID.c_str(), Reserve8.c_str(), TimeCondition.c_str(), GTDDate.c_str(), VolumeCondition.c_str(), MinVolume.c_str(), IsSwapOrder.c_str(), ForceCloseReason.c_str(), AccountID.c_str(), TradingDay.c_str());
}
int ItsInsertOrder::ToStream(char* buff, int size)
{
	return snprintf(buff, size, "%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|", ProtocolType.c_str(), Command.c_str(), SequenceNo.c_str(), Reserve1.c_str(), Reserve2.c_str(), Reserve3.c_str(), Reserve4.c_str(), Reserve5.c_str(), Reserve6.c_str(), ExchangeID.c_str(), ContractID.c_str(), Price.c_str(), Direction.c_str(), Reserve7.c_str(), CombHedgeFlag.c_str(), Volume.c_str(), OrderPriceType.c_str(), RequestID.c_str(), Reserve8.c_str(), TimeCondition.c_str(), GTDDate.c_str(), VolumeCondition.c_str(), MinVolume.c_str(), IsSwapOrder.c_str(), ForceCloseReason.c_str(), AccountID.c_str(), TradingDay.c_str());
}
string ItsInsertOrder::CreateSql()
{
	return "CREATE TABLE IF NOT EXISTS t_ItsInsertOrder(ProtocolType char(32), Command char(32), SequenceNo char(32), Reserve1 char(32), Reserve2 char(32), Reserve3 char(32), Reserve4 char(32), Reserve5 char(32), Reserve6 char(32), ExchangeID char(32), ContractID char(32), Price char(32), Direction char(32), Reserve7 char(32), CombHedgeFlag char(32), Volume char(32), OrderPriceType char(32), RequestID char(32), Reserve8 char(32), TimeCondition char(32), GTDDate char(32), VolumeCondition char(32), MinVolume char(32), IsSwapOrder char(32), ForceCloseReason char(32), AccountID char(32), TradingDay char(32), PRIMARY KEY(TradingDay, RequestID));";
}
string ItsInsertOrder::InsertSql()
{
	::memset(m_Buff, 0, sizeof(m_Buff));
	snprintf(m_Buff, 4096, "REPLACE INTO t_ItsInsertOrder VALUES(\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\", \"%s\");", ProtocolType.c_str(), Command.c_str(), SequenceNo.c_str(), Reserve1.c_str(), Reserve2.c_str(), Reserve3.c_str(), Reserve4.c_str(), Reserve5.c_str(), Reserve6.c_str(), ExchangeID.c_str(), ContractID.c_str(), Price.c_str(), Direction.c_str(), Reserve7.c_str(), CombHedgeFlag.c_str(), Volume.c_str(), OrderPriceType.c_str(), RequestID.c_str(), Reserve8.c_str(), TimeCondition.c_str(), GTDDate.c_str(), VolumeCondition.c_str(), MinVolume.c_str(), IsSwapOrder.c_str(), ForceCloseReason.c_str(), AccountID.c_str(), TradingDay.c_str());
	return string(m_Buff);
}
...
ItsInsertOrderCancel::ItsInsertOrderCancel(const vector<string>& items)
{
	...
	if(items.size() < 18)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsInsertOrderCancel's ColNum:[18]", items.size());
		return;
	}
	ProtocolType = items[0];
	Command = items[1];
...
{
	return snprintf(buff, size, "ItsInsertOrderCancel: ProtocolType[%s] Command[%s] SequenceNo[%s] Reserve1[%s] Reserve2[%s] Reserve3[%s] Reserve4[%s] Reserve5[%s] Reserve6[%s] OrderRef[%s] FrontID[%s] SessionID[%s] ExchangeID[%s] OrderSysID[%s] ChannelOrderID[%s] BrokerOrderID[%s] ContractID[%s] Direction[%s] TradingDay[%s] ", ProtocolType.c_str(), Command.c_str(), SequenceNo.c_str(), Reserve1.c_str(), Reserve2.c_str(), Reserve3.c_str(), Reserve4.c_str(), Reserve5.c_str(), Reserve6.c_str(), OrderRef.c_str(), FrontID.c_str(), SessionID.c_str(), ExchangeID.c_str(), OrderSysID.c_str(), ChannelOrderID.c_str(), BrokerOrderID.c_str(), ContractID.c_str(), Direction.c_str(), TradingDay.c_str());
}
...
ItsRspOrder::ItsRspOrder(const vector<string>& items)
{
	...
	if(items.size() < 16)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsRspOrder's ColNum:[16]", items.size());
		return;
	}
	ProtocolType = items[0];
	SequenceNo = items[1];
...
{
	return "CREATE TABLE IF NOT EXISTS t_ItsRspOrder(ProtocolType char(32), SequenceNo char(32), Reserve1 char(32), Reserve2 char(32), ErrorID char(32), ErrorMsg char(32), TradingDay char(32), PRIMARY KEY(TradingDay, SequenceNo));";
}
...
ItsOrder::ItsOrder(const vector<string>& items)
{
	...
	if(items.size() < 39)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsOrder's ColNum:[39]", items.size());
		return;
	}
	ProtocolType = items[0];
	Command = items[1];
...
ItsTrade::ItsTrade(const vector<string>& items)
{
	...
	if(items.size() < 26)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsTrade's ColNum:[26]", items.size());
		return;
	}
	ProtocolType = items[0];
	Command = items[1];
...
{
	return "CREATE TABLE IF NOT EXISTS t_ItsTrade(ProtocolType char(32), Command char(32), Reserve1 char(32), Reserve2 char(32), Reserve3 char(32), Reserve4 char(32), ChannelID char(32), ExchangeID char(32), ContractID char(32), OrderRef char(32), OrderSysID char(32), TradeTime char(32), TradeID char(32), Direction char(32), OffsetFlag char(32), HedgeFlag char(32), BrokerOrderID char(32), Volume char(32), TradeDate char(32), TradingDay char(32), Price char(32), Reserve5 char(32), TradeType char(32), ExchangeTradeID char(32), Reserve7 char(32), Reserve8 char(32), PRIMARY KEY(TradingDay, ExchangeID, ContractID, Direction, TradeID));";
}
...
ItsErrRtnOrderCancel::ItsErrRtnOrderCancel(const vector<string>& items)
{
	...
	if(items.size() < 16)
	{
		WRITE_LOG(LogLevel::Error, "Items Size:[%d] less than ItsErrRtnOrderCancel's ColNum:[16]", items.size());
		return;
	}
	ProtocolType = items[0];
	Command = items[1];
