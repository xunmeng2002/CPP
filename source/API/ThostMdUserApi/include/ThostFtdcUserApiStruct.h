...

{
	...
	TThostFtdcLargeVolumeType	Volume;
...
struct CThostFtdcQryExchangeStatusField
...
struct CThostFtdcExchangeStatusField

{
	...
	TThostFtdcDateType			ExchangeDate;

	///当前时间

	TThostFtdcTimeType			ExchangeTime;
...
struct CThostFtdcOptionsInfoField

{
	...
	TThostFtdcOptionsClassType  OptionsClass;

	///行权方式

	TThostFtdcOptionsModeType  OptionsMode;
	...
	TThostFtdcDateType  ExpiredDate;

	///行权比例

	TThostFtdcVolumeMultipleType StrikeUnit;

	///期权方向

	TThostFtdcOptionsDirectType OptionsDirect;
...
//StockInfo id=2  股票基本数据

struct CThostFtdcStockInfoField

{

	char szContractID[31];				//商品合约代码

	char szExchangeID[9];				//交易所代码（市场号转换）

	unsigned char byStopFlag;			//全天停牌标记

	double dAvg5Volume;					//5日均量

	double dTotalCapital;				//总股本

	double dFlowCapital;				//流通股本

	double dAvgNetAssets;				//每股净资产

	double dLastAvgProfit;				//上年度每股收益

	double dForecastAvgProfit;			//预计本年度每股收益

	double dNetProfit;					//净利润

	double dTotalAssets;				//总资产

	double dTotalDebt;					//总负债

};



//BondsInfo id=4 债券基础数据

struct CThostFtdcBondsInfoField

{
	...
	unsigned char byPaymentType;		//付息方式

	unsigned char byPaymentFreq;		//年付息频率

	int nIssueDate;						//发行日期

	int nListingDate;					//上市交易日

	int nInterestDate;					//开始计息日

	float fDuration;					//债券剩余期限（单位：年）

	float fIssuePrice;					//发行价

	double dFaceValue;					//发行面额

	float fFaceRate;					//票面利率%

};



//OptionsInfo id=5 期权基本信息

struct CThostFtdcOptionsInfoFieldEx

{
	...
	char szTargetSymbol[31];			//期权标的代码 510050.SHSE

	unsigned char byOptionsClass;		//期权类型

	unsigned char byOptionsMode;		//行权方式

	float fStrikePrice;					//行权价

	int nStrikeDate;					//行权到期日

	int nStrikeUnit;					//行权比例

	unsigned char byOptionsDirect;		//期权方向

	char cOptionsAdjust;				//合约调整

	char cOptionsLife;					//合约存续

	char cOptionsLimit;					//合约开仓限制

	char cAllowTactics;					//允许组合策略

	int nAutoSplitDate;					//垂直价差策略解除日期

};



//QuotationData id=10 行情快照

struct CThostFtdcQuotationDataField

{
	...
	int nTradeDate;						//交易日期 yyyymmdd

	int nUpdateDate;					//数据更新日期 yyyymmdd

	int nUpdateTime;					//数据更新时间 HHMMSSmmm

	double dLastClose;					//昨收价

	double dLastClear;					//昨结算

	double dLastOpenInterest;			//昨持仓
	...
	double dUpperLimit;					//涨停价

	double dLowerLimit;					//跌停价
...

	{
		...
		double dAskPrice;				//委卖价

		int nAskVolume;					//委卖量

	} Ask[10];

	int nTradeTicks;					//成交笔数

	unsigned char byTradeDirect;		//交易方向

	char cTradeStatus;					//交易状态

	double dFundsPV;					//基金前日净值

	double dFundsIOPV;					//基金实时参考净值

};



//OrderData id=11 委托队列明细

struct CThostFtdcOrderDataField

{
	...
	int nTradeDate;						//交易日 yyyymmdd

	int nUpdateDate;					//更新日期 yyyymmdd

	int nUpdateTime;					//更新时间 HHMMSSmmm

	unsigned char byOrderDirect;		//委托方向

	double dOrderPrice;					//委托价格

	int nOrderVolume[50];				//委托量

};



//IndexQuotationData id=12 指数快照

struct CThostFtdcIndexQuotDataField

{
	...
	unsigned short nCalcNum;			//统计样本数

	unsigned short nRaiseNum;			//上涨数

	unsigned short nDownNum;			//下跌数

	char szLeadStock[31];				//领涨股 600000.SHSE
